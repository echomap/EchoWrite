...
public class ProfilePersistV2 {
	...
	public Profile convertFromV1toV2(final ProfileData profileData) {
		...
		profile.setVolume(setupValue(profileData, "volume", ""));
		...
		profile.setTimeLineAddTimePerScene(getSafeBoolean(profileData, "cbTimeLineAddTimePerScene"));
		...
		profile.setOutputCharCardFile(setupValue(profileData, "outputFileTextCharCard", ""));
		profile.setTimelineTimePadding(setupValue(profileData, "timelineTimePadding", ""));

		return profile;
	}

	private boolean getSafeBoolean(final ProfileData profileData, final String key) {
		final Boolean bool = profileData.getSelected(key);
		if (bool == null)
			return false;
		return bool;
	}
	...
	public void setupDao(final FormatDao formatDao, final Profile selectedProfile, final String appVersion) {
		...
		if (selectedProfile.isTimeLineAddTimePerScene())
			formatDao.setTimeLineAddTimePerScene(true);
		...
		formatDao.setOutputCharCardFile(selectedProfile.getOutputCharCardFile());
		formatDao.setTimelineTimePadding(selectedProfile.getTimelineTimePadding());
	...
	public ProfileData convertFromV2toV1(Profile profile) {
		...
		profileData.setSelected("cbTimeLineAddTimePerScene", profile.isTimeLineAddTimePerScene());
		...
		//
		profileData.setText("outputFileTextCharCard", profile.getOutputCharCardFile());
		profileData.setText("timelineTimePadding", profile.getTimelineTimePadding());
