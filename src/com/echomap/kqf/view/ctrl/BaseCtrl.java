...
public abstract class BaseCtrl {
	...
	String loadPropFromAppOrDefault(final String key, final String defaultValue) {
		if (appProps != null && appProps.containsKey(Base.PROP_KEY_VERSION)) {
	...
	public static String getCurrentDateFmt() {
		...
		Base.myDateFormat.setTimeZone(cal.getTimeZone());
		String txt = Base.myDateFormat.format(cal.getTime());
	...
	boolean getSafeBooleanOrFalse(final Object val) {
		if (val == null)
			return false;
	...
	boolean getSafeBooleanOrTrue(final Object val) {
		if (val == null)
			return true;
	...
	void openNewWindow(String windowName, String windowTitle, final TextArea reportArea, Stage owner,
		...
		paramsMap.put("FRAMENAME", windowName);
		try {
			...
			final boolean modal = getSafeBooleanOrTrue(paramsMap.get(Base.PARAMMAP_MODAL));
			if (modal) {
	...
	protected String worktype() {
		return Base.WINDOWKEY_PROFILE_NEW;
	}
	...
	public void setupController(final Properties props, final Preferences appPreferences, final Stage primaryStage,
		...
		this.appVersion = appProps.getProperty(Base.PROP_KEY_VERSION);
		...
		if (appPreferences != null) {
			String prefKey = String.format("view/%s/%s", paramsMap.get("FRAMENAME"), Prefs.VIEW_WINDOW_Y);
			final double windowY = appPreferences.getDouble(prefKey, -1);
			prefKey = String.format("view/%s/%s", paramsMap.get("FRAMENAME"), Prefs.VIEW_WINDOW_X);
			final double windowX = appPreferences.getDouble(prefKey, -1);
			prefKey = String.format("view/%s/%s", paramsMap.get("FRAMENAME"), Prefs.VIEW_WINDOW_W);
			final double windowW = appPreferences.getDouble(prefKey, -1);
			prefKey = String.format("view/%s/%s", paramsMap.get("FRAMENAME"), Prefs.VIEW_WINDOW_H);
	...
	private void windowYChanged(final Number newVal) {
		if (appPreferences != null) {
			final String prefKey = String.format("view/%s/%s", paramsMap.get("FRAMENAME"), Prefs.VIEW_WINDOW_Y);
	...
	private void windowXChanged(final Number newVal) {
		if (appPreferences != null) {
			final String prefKey = String.format("view/%s/%s", paramsMap.get("FRAMENAME"), Prefs.VIEW_WINDOW_X);
			appPreferences.putDouble(prefKey, newVal.doubleValue());
		}
	}
	...
	private void windowHeightChanged(final Number newVal) {
		if (appPreferences != null) {
			final String prefKey = String.format("view/%s/%s", paramsMap.get("FRAMENAME"), Prefs.VIEW_WINDOW_H);
			appPreferences.putDouble(prefKey, newVal.doubleValue());
		}
	}
	...
	private void windowWidthChanged(final Number newVal) {
		if (appPreferences != null) {
			final String prefKey = String.format("view/%s/%s", paramsMap.get("FRAMENAME"), Prefs.VIEW_WINDOW_W);
			appPreferences.putDouble(prefKey, newVal.doubleValue());
		}
	}
	...
	protected File locateFile(final ActionEvent event, final String title, final TextField textField,
			final String defaultName, final Base.FILTERTYPE defaultExtension) {
	...
	File chooseFile(final ActionEvent event, final String title, final TextField textField, final String defaultName,
			final Base.FILTERTYPE defaultExtension) {
		final FileChooser chooser = new FileChooser();
		File startDir = null;
		if (textField != null && !StringUtils.isEmpty(textField.getText())) {
			startDir = new File(textField.getText());
	...
	public void handleRunTimeline(final BaseCtrl baseCtrl, final ProfileManager profileManager,
		...
		try {
			...
			openNewWindow(Base.WINDOWKEY_TIMELINE, windowTitle, loggingText, primaryStage, this, notifyCtrl, paramsMap);
	...
	public void doSceneHiding() {
	}

	public void doSceneShowing() {
		windowCount += 1;
		LOGGER.debug("windowCount=" + windowCount);
	}
