...
public class CtrlProfileEdit extends BaseCtrl implements Initializable, WorkFinishedCallback {
	...
	@FXML
	private CheckBox cbTimeLineAddTimePerScene;

	@FXML
	private TextField outputOutlineCSVFileText;
	@FXML
	private TextField outputOutlineAllCSVFileText;
	@FXML
	private TextField outputDocTagsOutlineFileText;
	@FXML
	private TextField outputDocTagsSceneFileText;
	@FXML
	private TextField outputFileTextCharCard;

	@FXML
	private TextField timelineTimePadding;
	...
	public void handleRunOutliner(final ActionEvent event) {
		...
		try {
			...
			final BaseRunner br = new BaseRunner();
			br.handleRunOutline(this, profileManager, this.selectedProfile, lastRunText, myWorkDoneNotify);
		} catch (Exception e) {
			e.printStackTrace();
			btnRunOutliner.setDisable(false);
		}
	...
	//
	public void handleBrowseCharCardFile(final ActionEvent event) {
		LOGGER.debug("handleBrowseCharCardFile: Called");
		final String inFileName = getPrefixedFileName("CharFile", "txt");
		chooseFile(event, "CharCard File", outputFileTextCharCard, inFileName, FILTERTYPE.TEXT);
		LOGGER.debug("handleBrowseCharCardFile: Done");
	}

	private String getPrefixedFileName(final String postfix, final String ext) {
		final String filePrefixText = selectedProfile.getInputFilePrefix();
		if (StringUtils.isBlank(filePrefixText)) {
			return getInputFileName(postfix, ext);
		}

		final File inFile = new File(selectedProfile.getInputFile());
		final StringBuilder sbuf = new StringBuilder();
		if (inFile.exists() && inFile.getParentFile().exists()) {
			sbuf.append(inFile.getParentFile().getAbsolutePath());
			sbuf.append(File.separator);
		}
		sbuf.append(filePrefixText);
		if (selectedProfile.isAppendUnderscoreToPrefix())
			sbuf.append("_");
		sbuf.append(postfix);
		sbuf.append(".");
		sbuf.append(ext);

		final File ofile = new File(sbuf.toString());
	...
	private void setFieldsFromProfile() {
		...
		cbTimeLineAddTimePerScene.setSelected(child.isTimeLineAddTimePerScene());
		...
		outputFileTextCharCard.setText(child.getOutputCharCardFile());
		timelineTimePadding.setText(child.getTimelineTimePadding());
		//
	...
	private Profile setProfileFromFields() {
		...
		child.setTimeLineAddTimePerScene(cbTimeLineAddTimePerScene.isSelected());
		// cbCenterable.setSelected(child.isCbCenterable());
		...
		child.setOutputCharCardFile(outputFileTextCharCard.getText());
		child.setTimelineTimePadding(timelineTimePadding.getText());
